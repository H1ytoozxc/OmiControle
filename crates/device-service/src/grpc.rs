...
impl pb::device_service_server::DeviceService for DeviceGrpc {
    async fn create_enrollment(
        ...
        sqlx::query!(
            r#"INSERT INTO device.enrollment_codes (code, tenant_id, issued_by_user, expires_at, labels)
               VALUES ($1,$2,$3,$4,$5)"#,
            code, tenant_id, issued_by_user, expires_at, labels
        )
    ...
    async fn delete_device(&self, req: Request<pb::DeleteDeviceRequest>) -> Result<Response<pb::Empty>, Status> {
        ...
        sqlx::query!("UPDATE device.devices SET deleted_at = now() WHERE id = $1", id)
    ...
    async fn enroll(&self, req: Request<pb::EnrollRequest>) -> Result<Response<pb::EnrollResponse>, Status> {
        ...
        sqlx::query!(
            ...
            device_id, row.tenant_id, r.hostname, r.platform, r.agent_version, r.public_key, row.labels
        )
    ...
    async fn channel(
        ...
        tokio::spawn(async move {
            // Mark device online.
            let _ = sqlx::query!(
                r#"UPDATE device.devices SET status = 'online', last_seen_at = now()
                   WHERE id = $1"#,
                Uuid::parse_str(&dev_for_task).unwrap_or(Uuid::nil())
            ).execute(&pool).await;
