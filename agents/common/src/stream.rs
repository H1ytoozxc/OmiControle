...
impl ChannelClient {
    ...
    async fn try_run(
        ...
        // x-device-id stamped by an upstream interceptor or set explicitly:
        // req.metadata_mut().insert("x-device-id", ...);
        ...
        while let Some(msg) = server_stream.message().await? {
            if inbound.send(msg).await.is_err() { break; }
        }
        Ok(())
    }
}
