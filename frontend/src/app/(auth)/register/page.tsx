    ...
    <div className="min-h-screen grid grid-cols-1 lg:grid-cols-2 bg-ink">
        ...
        <div className="space-y-8">
          <div>
            ...
            <p className="text-[14px] text-bone-muted mt-4 max-w-[360px] leading-relaxed">
              Free to start. Enroll unlimited devices, create workflows, and deploy AI agents.
            </p>
          </div>
      ...
      <main className="flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-[380px] fade-up">
          ...
          {step === "account" && <AccountStep onNext={() => setStep("org")} showPw={showPw} setShowPw={setShowPw} />}
          {step === "org"     && <OrgStep     onNext={() => setStep("done")} onBack={() => setStep("account")} />}
          {step === "done"    && <DoneStep />}
        </div>
      </main>
    </div>
...
function AccountStep({ onNext, showPw, setShowPw }: { onNext: () => void; showPw: boolean; setShowPw: (v: boolean) => void }) {
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-[24px] font-semibold text-bone tracking-tight">Create your account</h2>
        <p className="text-[13px] text-bone-muted mt-1">
          Already have one?{" "}
          <Link href="/login" className="text-ember hover:text-ember-50 transition-colors">Sign in</Link>
        </p>
      </div>

      <div className="space-y-2">
        <Button variant="plate" size="lg" className="w-full justify-between">
          <span className="flex items-center gap-2.5"><Github className="w-4 h-4" strokeWidth={1.6} />Continue with GitHub</span>
          <ArrowRight className="w-3.5 h-3.5 text-bone-dim" strokeWidth={1.6} />
        </Button>
        <Button variant="plate" size="lg" className="w-full justify-between">
          <span className="flex items-center gap-2.5"><Mail className="w-4 h-4" strokeWidth={1.6} />Continue with Google</span>
          <ArrowRight className="w-3.5 h-3.5 text-bone-dim" strokeWidth={1.6} />
        </Button>
      </div>
      ...
      <p className="text-[11px] font-mono text-bone-dim text-center leading-relaxed">
        ...
        <Link href="#" className="text-bone-muted hover:text-bone transition-colors">Terms</Link>
        {" "}and{" "}
        <Link href="#" className="text-bone-muted hover:text-bone transition-colors">Privacy Policy</Link>.
      </p>
    </div>
  );
}

function OrgStep({ onNext, onBack }: { onNext: () => void; onBack: () => void }) {
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-[24px] font-semibold text-bone tracking-tight">Set up your organization</h2>
        <p className="text-[13px] text-bone-muted mt-1">This becomes your tenant namespace.</p>
      </div>
      ...
      <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); onNext(); }}>
        ...
        <label className="flex flex-col gap-1.5">
          <span className="eyebrow">Tenant slug</span>
          ...
          <span className="text-[10.5px] font-mono text-bone-dim">Lowercase letters, numbers, and hyphens only.</span>
        </label>
...
function DoneStep() {
  return (
    <div className="space-y-5 text-center">
      ...
      <div>
        <h2 className="text-[24px] font-semibold text-bone tracking-tight">You&apos;re all set</h2>
        <p className="text-[13px] text-bone-muted mt-1 max-w-[280px] mx-auto leading-relaxed">
          Your organization is ready. Enroll your first device to get started.
        </p>
      </div>
