...
const T = {
  en: {
    ...
    onboarding: {
      ...
      title: "Install the Sequoia agent",
      subtitle: "Run this command on the machine you want to monitor. The agent enrolls automatically using your tenant token.",
      iRanIt: "I ran the command — waiting for device",
      ...
      // Command explanation
      whatLabel: "What does this command do?",
      explainIntro: "This one-liner downloads and installs the Sequoia agent on the target machine.",
      curlLabel: "curl -fsSL https://get.sequoia.io/agent",
      curlDesc: "Downloads the install script (~8 KB) over HTTPS from Sequoia's servers. Flags: -f (fail on HTTP errors), -s (silent), -S (show errors), -L (follow redirects).",
      pipeLabel: "| sudo bash",
      pipeDesc: "Pipes the script directly into bash as root. Required because the agent writes to system directories.",
      installsLabel: "What gets installed:",
      installItems: [
        "/usr/local/bin/sequoia-agent — agent binary (~12 MB)",
      ...
      afterDesc: "The agent starts automatically, connects to your tenant's control plane, and shows up in Devices within ~30 seconds.",
      // Steps
      stepAccount: "Account",
  ...
  ru: {
    ...
    onboarding: {
      ...
      title: "Установите агент Sequoia",
      subtitle: "Запустите эту команду на машине, которую хотите мониторить. Агент автоматически зарегистрируется с токеном вашего тенанта.",
      iRanIt: "Команда выполнена — жду устройство",
      ...
      whatLabel: "Что делает эта команда?",
      explainIntro: "Эта однострочная команда скачивает и устанавливает агент Sequoia на целевую машину.",
      curlLabel: "curl -fsSL https://get.sequoia.io/agent",
      curlDesc: "Скачивает установочный скрипт (~8 КБ) по HTTPS с серверов Sequoia. Флаги: -f — не выводить страницы ошибок HTTP, -s — тихий режим, -S — показывать ошибки, -L — следовать редиректам.",
      pipeLabel: "| sudo bash",
      pipeDesc: "Передаёт скрипт напрямую в bash с правами root. Необходимо, так как агент пишет в системные директории.",
      installsLabel: "Что устанавливается:",
      installItems: [
        "/usr/local/bin/sequoia-agent — бинарный файл агента (~12 МБ)",
      ...
      afterDesc: "Агент запускается автоматически, подключается к управляющей плоскости вашего тенанта и появляется в списке устройств примерно через 30 секунд.",
      stepAccount: "Аккаунт",
      stepOrg: "Организация",
