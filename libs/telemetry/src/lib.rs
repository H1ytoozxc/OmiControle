...
use opentelemetry_sdk::trace::{BatchConfig, Config, Sampler};
...
pub fn init(cfg: TelemetryConfig) -> Result<TelemetryGuard, TelemetryError> {
    ...
    let resource = Resource::new(vec![
        KeyValue::new(opentelemetry_semantic_conventions::resource::SERVICE_NAME, cfg.service_name.clone()),
        KeyValue::new(opentelemetry_semantic_conventions::resource::SERVICE_NAMESPACE, cfg.service_namespace.clone()),
        KeyValue::new(
    ...
    let tracer_provider = if let Some(endpoint) = cfg.otlp_endpoint.as_deref() {
        opentelemetry_otlp::new_pipeline()
            ...
            .map_err(|e| TelemetryError::Init(format!("otlp: {e}")))?
    } else {
        // No OTLP endpoint => stdout exporter (dev mode).
